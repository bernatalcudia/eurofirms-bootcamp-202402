...
describe("toggleLikeProduct", () => {
    ...
    it("likes a product", () => {
        ...
        return User.create({
            ...
            .then(() => toggleLikeProduct(user!._id.toString(), product!._id.toString()))
            ...
            .finally(() => {
                ...
                expect(product?.likes.length).to.equal(1)
                expect(product?.likes[0].toString()).to.equal(user!._id.toString())
    ...
    it("product not found", () => {
        ...
        return User.create({ name: "pepito", birthdate: new Date("1970-05-07"), email: "pepito@gmail.com", username: "pepito", password: bcrypt.hashSync("123123123", 10) })
            .then(_user => {
                user = _user
                return toggleLikeProduct(user!._id.toString(), "68c2e427cbfeb33bb36e1e85")
    ...
    it("user not found", () => {
        let error: Error
        return toggleLikeProduct("663ccaeac792d77a1492d494", "68c2e427cbfeb33bb36e1e85")
            .catch(_error => (error = _error))
            .finally(() => {
                expect(error).to.be.an.instanceOf(NotFoundError)
